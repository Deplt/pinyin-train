(() => {
  ...
  function stickersPage() {
    return `<main><div class="page-heading"><div class="eyebrow">我的森林收藏</div><h1>每一张贴纸，都是一次小进步</h1><p>完成一趟小火车，就有一位新朋友来报到。</p></div><div class="collection-summary"><span aria-hidden="true">🎒</span><div><h2>已经认识 ${progress.stickers.length} / ${D.stickers.length} 位森林朋友</h2><p>完成 ${progress.trips} 趟旅程，收集 ${progress.stars} 颗星星。${progress.stickers.length===12?'朋友们都到齐啦！':'下一位朋友，正在车站等你。'}</p></div></div><div class="sticker-grid">${D.stickers.map((s,i) => {const unlocked=progress.stickers.includes(s.id);return `<div class="sticker ${unlocked?'':'locked'}"><div class="sticker-emoji" aria-hidden="true">${unlocked?s.emoji:'?'}</div><strong>${unlocked?s.name:`第 ${i+1} 位朋友`}</strong><small>${unlocked?'已经成为好朋友':`完成第 ${i+1} 趟获得`}</small></div>`;}).join('')}</div><div class="departure"><div class="departure-copy"><span class="emoji">🚂</span><div><strong>一起去森林里转转吧</strong><p>答错不扣星星，用提示也能获得奖励。</p></div></div><button class="primary" data-action="nav" data-view="map">去接小乘客${icon('arrow')}</button></div>${storageAvailable?'':'<p class="audio-warning">这个浏览器暂时不能保存记录；本次仍可正常收集，关闭后可能丢失。</p>'}</main>`;
  }
  function bookPage() {
