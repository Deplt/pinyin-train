(() => {
  ...
  function header() {
    ...
    const navView=['stickers','game','reward'].includes(view)?'map':view;
    ...
    return `<header class="topbar">
      ...
      <nav class="nav" aria-label="主要导航">${nav.map(([key,i,label]) => `<button data-action="nav" data-view="${key}" class="${navView === key ? 'active' : ''}" ${navView===key?'aria-current="page"':''}>${icon(i)}${label}</button>`).join('')}</nav>
  ...
  function mapPage() {
    ...
    return `<main>
      ...
      <div class="train-collection"><span class="train-album-icon" aria-hidden="true">🎒</span><div><strong>小火车的动物收藏册</strong><p>已经认识 ${progress.stickers.length} / ${D.stickers.length} 位朋友</p></div><button class="secondary" data-action="nav" data-view="stickers">我的贴纸 →</button></div>
